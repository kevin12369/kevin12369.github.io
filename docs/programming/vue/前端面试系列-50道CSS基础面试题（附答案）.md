...
- border-box：IE传统盒子模型。设置元素的height/width属性指的是border + padding + content部分高/宽
## CSS选择器有哪些？哪些属性可以继承？
...
3. word-wrap（对长的不可分割单吃换行） word-wrap：break-word
...
该布局模型的目的时提供一种更加高效的方式来对容器中的条目进行布局、对其和分配空间。
在传统的布局方式中，