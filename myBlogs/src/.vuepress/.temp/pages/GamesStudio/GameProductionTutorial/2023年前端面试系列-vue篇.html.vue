...
<ul>
<li>
<p><code v-pre>默认插槽</code>：<br>
子组件用<code v-pre>&lt;slot&gt;</code>标签来确定渲染的位置，标签里面可以放<code v-pre>DOM</code>结构作为后备内容，当父组件在使用的时候，可以直接在子组件的标签内写入内容，该部分内容将插入子组件的<code v-pre>&lt;slot&gt;</code>标签位置。<br>
如果父组件使用的时候没有往插槽传入内容，后备内容就会显示在页面。</p>
</li>
<li>
<p><code v-pre>具名插槽</code>：<br>
子组件用<code v-pre>name</code>属性来表示插槽的名字，没有制定<code v-pre>name</code>的插槽，会有隐含的名称叫做<code v-pre>default</code>。<br>
父组件在使用时，在默认插槽的基础上通过<code v-pre>v-slot</code>指令制定元素需要放在那个插槽中，<code v-pre>v-slot</code>值为子组件插槽<code v-pre>name</code>属性值。<br>
