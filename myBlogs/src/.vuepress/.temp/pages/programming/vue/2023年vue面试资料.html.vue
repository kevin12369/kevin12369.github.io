...
<h2 id="vue中key的作用" tabindex="-1"><a class="header-anchor" href="#vue中key的作用" aria-hidden="true">#</a> Vue中key的作用？</h2>
<ul>
...
<li>
<p>工作原理：Vue.js内部将DOM节点，抽象成了一个个的VNode节点，keep-alive组件的缓存也是基于VNode节点的。它将满足条件的组件在cache对象中缓存起来，等到重新渲染的时候在将VNode节点从cache对象中取出并渲染。</p>
</li>
<li>
<p>可以设置一下属性：</p>
<ul>
<li>
<p>include：字符串或者正则，只有名字匹配的组件才会被缓存。</p>
</li>
<li>
<p>exclude：字符串或者正则，任何名字匹配的组件都不会被缓存。</p>
</li>
<li>
<p>max：数字，最多可以缓存多少组件实例。</p>
</li>
</ul>
<p>匹配首先检查组价的name选项，如果name选项不可用，则匹配它的局部注册名称（父组件components选项的键值），匿名组件不能被匹配。</p>
</li>
</ul>
<p>设置了keep-alive缓存的组件会多出两个生命周期钩子：actived、deactived。</p>
</div></template>
