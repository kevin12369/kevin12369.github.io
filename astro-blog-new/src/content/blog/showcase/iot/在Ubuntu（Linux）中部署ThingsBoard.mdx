...
description: '详细介绍如何在 Ubuntu Linux 系统上部署 ThingsBoard 物联网平台，包括环境准备、Java 和 PostgreSQL 安装、ThingsBoard 配置及启动等完整步骤。'
...
<Label>步骤2：安装PostgreSQL数据库</Label>
...
<Label>步骤3：下载ThingsBoard安装包</Label>
...
<Aside>
**注意**：版本号可能需要根据实际情况调整，访问 [ThingsBoard GitHub Releases](https://github.com/thingsboard/thingsboard/releases) 查看最新版本。
</Aside>

## 安装 ThingsBoard

<Label>步骤4：安装ThingsBoard</Label>
...
<Label>步骤5：配置ThingsBoard数据库连接</Label>
...
<Label>步骤7：启动ThingsBoard服务</Label>
...
<Label>步骤8：访问ThingsBoard管理界面</Label>
...
ThingsBoard 默认使用 1GB 内存，如果服务器内存不足，可以在配置文件中调整：
...
更新 ThingsBoard 时，先备份数据，然后下载新版本并重新安装。
